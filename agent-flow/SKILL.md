...
4. **Execute os times** (Agent tool): independentes em paralelo, dependentes em sequência, em background. Cada agente recebe o objetivo do time, os arquivos/pastas sob sua responsabilidade e a instrução de devolver um resumo curto das entregas. Nunca dois times editando os mesmos arquivos ao mesmo tempo.
...
6. **No final:** dashboard completo (todos ✅, com duração e entregas por time) + resumo no chat com o link.

## Regras

- Tarefa pequena (um arquivo, ajuste simples) não justifica times: faça direto e diga por quê em uma linha.
- Os times de Segurança e Revisão rodam DEPOIS dos times de código, sobre o diff real — nunca em paralelo com quem está escrevendo.
- Cada agente é uma sessão nova com custo próprio: 3 a 5 times é o teto sensato para tarefas médias; avise o usuário antes de passar disso.
